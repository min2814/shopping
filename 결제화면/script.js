document.addEventListener('DOMContentLoaded', function () {
  ...
  const initiallyCheckedLabel = document.querySelector('input[name="payment-method"]:checked').parentElement;
  ...
  paymentButton.addEventListener('click', function () {
    ...
    const termsChecked = document.querySelector('.payment-terms input\[type="checkbox"]').checked;

    const selectedPaymentInput = document.querySelector('input[name="payment-method"]:checked');
    let paymentMethodText = selectedPaymentInput ? selectedPaymentInput.nextElementSibling.textContent : '선택 안 함';
    ...
    if (name && address && phone && termsChecked) {
      ...
      if (name && address && phone && !termsChecked) {
        message = '결제 서비스 이용 약관 및 개인정보 처리 동의에 체크해 주세요.';
      } else if (!(name && address && phone) && termsChecked) {
        message = '배송 정보를 모두 입력해 주세요.';
      } else if (!(name && address && phone) && !termsChecked) {
        message = '배송 정보를 모두 입력해 주시고, 결제 서비스 이용 약관 및 개인정보 처리 동의에 체크해 주세요.';
      }
      alert(message);
    }
  });
